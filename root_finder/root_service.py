        ...
        try:
            ...
            try:
                ...
                return render_template(
                    "error.html", error=f"Invalid function syntax: {str(e)}"
                )

            f = sp.lambdify(x, f_expr, "numpy")
            df = sp.lambdify(x, sp.diff(f_expr, x), "numpy")

            root_bisect, iter_bisect, (bisect_vals, bisect_errors) = bisection_method(
                f, a, b, tol
            )
            root_newton, iter_newton, (newton_vals, newton_errors) = newton_method(
                f, df, (a + b) / 2, tol
            )
